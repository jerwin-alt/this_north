...
import {
  ...
  Image, Platform, StyleSheet, Dimensions,
} from "react-native";
...
interface CakeSize  { id: number; size_name: string; size_inches: number; price_modifier: number; is_active: boolean; }
interface CakeFlavor{ id: number; flavor_name: string; is_active: boolean; }
interface Product   {
  id: number; name: string; description: string; base_price: number;
  menu_type: "standard" | "customizable"; has_size_options: boolean;
  category_id: number; image_url?: string;
  drinkSizes?: { id: number; size_name: string; price_modifier: number }[];
}
interface CartItem  {
  product: Product; quantity: number;
  sizeId?: number; sizeName?: string; sizePrice?: number;
  cakeSizeId?: number; cakeSizeName?: string; cakeSizePrice?: number;
  flavorId?: number; flavorName?: string;
}
interface Order {
  id: number; order_number: string; total_amount: number; status: string;
  pickup_date: string; created_at: string;
  items?: Array<{ menu: { name: string; image_url?: string; base_price: number }; quantity: number; unit_price: number }>;
}
...
function ProductImage({ imageUrl, size = 100 }: { imageUrl?: string; size?: number }) {
  ...
  if (!uri || err) {
    return (
      <View style={[s.imgFallback, { width: size, height: size, borderRadius: size * 0.16 }]}>
...
export default function CustomerDashboard() {
  ...
  const [addons, setAddons] = useState({ sizeId: undefined as number | undefined, cakeSizeId: undefined as number | undefined, flavorId: undefined as number | undefined, quantity: 1 });
  ...
  const fetchCategories = async () => {
    ...
    } catch (err) { console.error(err); Alert.alert("Error", "Failed to load categories"); }
  };
  ...
  const fetchCakeSizes = async () => {
    ...
    } catch (err) { console.error("Failed to fetch cake sizes", err); }
  };
  const fetchCakeFlavors = async () => {
    ...
    } catch (err) { console.error("Failed to fetch cake flavors", err); }
  };
  const fetchNotifications = async () => { setNotifications([]); };

  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    await Promise.allSettled([fetchOrders(), fetchNotifications(), fetchCategories(), fetchCakeSizes(), fetchCakeFlavors()]);
    setRefreshing(false);
  }, []);

  useFocusEffect(useCallback(() => { if (activeTab === "orders") fetchOrders(); }, [activeTab]));
  useEffect(() => { fetchCategories(); fetchOrders(); fetchCakeSizes(); fetchCakeFlavors(); }, []);
  useEffect(() => { if (selectedCategory) fetchProducts(); }, [selectedCategory]);
  ...
  const addToCart = () => {
    ...
    const item: CartItem = {
      product: selectedProduct, quantity: addons.quantity,
      sizeId:      selectedProduct.has_size_options ? addons.sizeId : undefined,
      sizeName:    selectedProduct.has_size_options && addons.sizeId ? selectedProduct.drinkSizes?.find(s => s.id === addons.sizeId)?.size_name : undefined,
      sizePrice:   selectedProduct.has_size_options && addons.sizeId ? selectedProduct.drinkSizes?.find(s => s.id === addons.sizeId)?.price_modifier || 0 : 0,
  ...
  const placeOrder = async () => {
    if (cart.length === 0)              { Alert.alert("Cart empty", "Please add items to your order"); return; }
    if (!pickupDate || !pickupTime)     { Alert.alert("Missing schedule", "Please select pickup date and time"); return; }
    setPlacingOrder(true);
    const orderItems = cart.map(item => ({ menu_id: item.product.id, quantity: item.quantity, size_id: item.sizeId, cake_size_id: item.cakeSizeId, flavor_id: item.flavorId }));
    const formattedTime = `${pickupTime.getHours().toString().padStart(2,"0")}:${pickupTime.getMinutes().toString().padStart(2,"0")}:00`;
    try {
      await axios.post("/customer/orders", { items: orderItems, pickup_date: pickupDate.toISOString().split("T")[0], pickup_time: formattedTime, notes: orderNotes });
      Alert.alert("Order Placed! 🎉", "Your order has been submitted for approval.");
      setCart([]); setShowCart(false); setOrderNotes(""); fetchOrders();
    } catch (err: any) {
                ...
                {filtered.length === 0 ? (
                  <View style={s.emptyWrap}>
                    <View style={s.emptyIconBox}>
                      <MaterialCommunityIcons name="cake-variant-outline" size={32} color={MUTED_GRAY} style={{ opacity: 0.5 }} />
                    </View>
                  ...
                  filtered.map(product => (
                    <TouchableOpacity
                      ...
                      activeOpacity={0.88}
                      onPress={() => {
                        setAddons({ sizeId: undefined, cakeSizeId: undefined, flavorId: undefined, quantity: 1 });
                    ...
                    >
                      ...
                      <View style={s.productInfo}>
                        ...
                        <Text style={s.productPrice}>₱{parseFloat(String(product.base_price)).toLocaleString()}</Text>
                      </View>
          ...
          <View style={s.tabContent}>
            ...
            {loadingOrders ? (
              ...
              <View style={s.emptyWrap}>
                ...
                <Text style={s.emptySubText}>Add items from the menu to place your first order</Text>
              </View>
            ) : (
              orders.map(order => (
                <View key={order.id} style={s.orderCard}>
                  {/* Card top accent */}
                  <View style={[s.orderCardAccent, { backgroundColor: STATUS_COLOR[order.status] || MUTED_GRAY }]} />
                      ...
                      {order.items.map((item, idx) => (
                        <View key={idx} style={s.orderItemRow}>
                          ...
                          <View style={{ flex: 1, marginLeft: 10 }}>
                            ...
                            <Text style={s.orderItemQty}>Qty: {item.quantity} × ₱{item.unit_price}</Text>
                          </View>
                        </View>
                      ))}
                    </View>
                  ...
                  <View style={s.orderCardBottom}>
                    <Text style={s.orderTotal}>₱{parseFloat(String(order.total_amount)).toLocaleString()}</Text>
      ...
      case "notifications":
        return (
          <View style={s.tabContent}>
            ...
            <View style={s.emptyWrap}>
              <View style={s.emptyIconBox}>
                <Ionicons name="notifications-off-outline" size={30} color={MUTED_GRAY} style={{ opacity: 0.5 }} />
              </View>
              ...
              <View style={s.verifCard}>
                <View style={s.verifCardLeft}>
                  <MaterialCommunityIcons
                    name={user.verification_type === "senior_citizen" ? "account-star" : "wheelchair-accessibility"}
                    size={20}
                    color={SAGE}
                  />
  ...
  return (
    <SafeAreaView style={s.root} edges={["top", "bottom"]}>
      ...
      <ScrollView
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={SAGE} colors={[SAGE]} />}
          ...
          return (
            ...
            >
              <View style={[s.tabIconWrap, active && s.tabIconWrapActive]}>
                <Ionicons name={(active ? tab.activeIcon : tab.icon) as any} size={20} color={active ? "#fff" : MUTED_GRAY} />
              </View>
        ...
        <View style={s.modalOverlay}>
          <View style={s.productModal}>
            ...
            <LinearGradient colors={[SAGE, SAGE_DARK]} style={s.modalHeader}>
              ...
              <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.modalPrice}>₱{selectedProduct && parseFloat(String(selectedProduct.base_price)).toLocaleString()}</Text>
                </View>
            ...
            <ScrollView style={{ padding: 20 }} showsVerticalScrollIndicator={false}>
              ...
              {/* Drink sizes */}
              {selectedProduct?.has_size_options && selectedProduct.drinkSizes && selectedProduct.drinkSizes.length > 0 && (
                <View style={s.optionSection}>
              ...
              <View style={s.qtyRow}>
                ...
                <View style={s.qtyControls}>
                  <TouchableOpacity onPress={() => setAddons({ ...addons, quantity: Math.max(1, addons.quantity - 1) })} style={s.qtyBtn}>
                    <Ionicons name="remove" size={18} color={SAGE} />
                  </TouchableOpacity>
                  <Text style={s.qtyValue}>{addons.quantity}</Text>
                  <TouchableOpacity onPress={() => setAddons({ ...addons, quantity: addons.quantity + 1 })} style={s.qtyBtn}>
              ...
              <View style={{ flexDirection: "row", gap: 10, marginTop: 20, marginBottom: 10 }}>
                <TouchableOpacity onPress={() => setProductModalVisible(false)} style={s.modalCancelBtn}>
                  <Text style={s.modalCancelText}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={addToCart} style={s.modalAddBtn} activeOpacity={0.88}>
                  <LinearGradient colors={[SAGE, SAGE_DARK]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.modalAddBtnGrad}>
                ...
                return (
                  <View style={s.cartItem}>
                    ...
                    <View style={{ flex: 1, marginLeft: 10 }}>
                      ...
                      {item.cakeSizeName && <Text style={s.cartItemMeta}>Cake: {item.cakeSizeName}</Text>}
                      {item.flavorName   && <Text style={s.cartItemMeta}>Flavor: {item.flavorName}</Text>}
                      <Text style={s.cartItemMeta}>Qty: {item.quantity} × ₱{item.product.base_price}</Text>
                    </View>
              ...
              ListFooterComponent={
                <View style={{ padding: 16 }}>
                  ...
                  <TouchableOpacity onPress={() => setShowTimePicker(true)} style={s.schedulePill}>
                    ...
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                      <Text style={s.schedulePillValue}>{pickupTime.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</Text>
                      <Ionicons name="chevron-forward" size={14} color={MUTED_GRAY} />
                    </View>
                  </TouchableOpacity>
                  ...
                  {/* Place order */}
                  <TouchableOpacity onPress={placeOrder} disabled={placingOrder} activeOpacity={0.88} style={[s.placeOrderBtn, placingOrder && { opacity: 0.7 }]}>
                    <LinearGradient colors={[SAGE, SAGE_DARK]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={s.placeOrderGrad}>
                      {placingOrder
      ...
      {showDatePicker && (
        <DateTimePicker
          value={pickupDate} mode="date" minimumDate={new Date()}
          display={Platform.OS === "ios" ? "spinner" : "default"}
          onChange={(event, selectedDate) => {
            setShowDatePicker(false);
            if (selectedDate) { setPickupDate(selectedDate); Alert.alert("Date updated", `New pickup date: ${selectedDate.toDateString()}`); }
          }}
        />
      )}
      ...
      {showTimePicker && (
        <DateTimePicker
          value={pickupTime} mode="time"
          display={Platform.OS === "ios" ? "spinner" : "default"}
          onChange={(event, selectedTime) => {
            setShowTimePicker(false);
            if (selectedTime) { setPickupTime(selectedTime); Alert.alert("Time updated", `New pickup time: ${selectedTime.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`); }
          }}
        />
      )}
    </SafeAreaView>
...
const s = StyleSheet.create({
  ...
  // Header
  header: { paddingTop: 8, paddingBottom: 16, paddingHorizontal: 20, position: "relative", overflow: "hidden" },
  headerBlob1: { position: "absolute", width: 180, height: 180, borderRadius: 999, backgroundColor: "rgba(255,255,255,0.06)", top: -60, right: -50 },
  headerBlob2: { position: "absolute", width: 100, height: 100, borderRadius: 999, backgroundColor: "rgba(255,255,255,0.04)", bottom: -30, left: -20 },
  ...
  headerAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: "rgba(255,243,217,0.2)", borderWidth: 1.5, borderColor: "rgba(255,243,217,0.3)", alignItems: "center", justifyContent: "center" },
  headerAvatarText: { fontSize: 14, fontWeight: "800", color: "#fff" },
  headerStamps: { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "rgba(255,255,255,0.1)", borderRadius: 999, paddingHorizontal: 12, paddingVertical: 5, alignSelf: "flex-start" },
  headerStampsText: { fontSize: 12, fontWeight: "600", color: "rgba(255,243,217,0.85)" },

  // Category tabs
  catScroll: { paddingVertical: 14 },
  catPill: { paddingHorizontal: 18, paddingVertical: 8, borderRadius: 999, backgroundColor: "#fff", marginRight: 8, borderWidth: 1.5, borderColor: "rgba(166,162,154,0.2)" },
  catPillActive: { backgroundColor: SAGE, borderColor: SAGE, shadowColor: SAGE, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.25, shadowRadius: 8, elevation: 4 },
  catPillText: { fontSize: 13, fontWeight: "600", color: MUTED_GRAY },
  catPillTextActive: { color: "#fff" },

  // Count label
  countLabel: { fontSize: 11, fontWeight: "600", color: MUTED_GRAY, letterSpacing: 0.4, paddingHorizontal: 16, marginBottom: 8 },

  // Products grid
  productGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", paddingHorizontal: 14 },
  productCard: {
    width: "48%", backgroundColor: "#fff", borderRadius: 18,
    marginBottom: 14, overflow: "hidden",
    borderWidth: 1.5, borderColor: "rgba(242,237,228,0.9)",
    shadowColor: SAGE, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 8, elevation: 3,
  },
  productCardAccent: { height: 3, backgroundColor: `linear-gradient(90deg, ${SAGE}, ${SAGE_DARK})`, backgroundColor: SAGE, opacity: 0.6 },
  ...
  productName: { fontSize: 13, fontWeight: "700", color: SAGE, letterSpacing: -0.2, lineHeight: 18, marginBottom: 2 },
  ...
  customBadge: { position: "absolute", bottom: 2, right: 8, backgroundColor: SOFT_WHITE, borderRadius: 999, paddingHorizontal: 7, paddingVertical: 2, borderWidth: 1, borderColor: "rgba(212,160,61,0.3)" },
  customBadgeText: { fontSize: 9, fontWeight: "700", color: "#92670a" },
  productAddBtn: { position: "absolute", bottom: 10, right: 10, width: 24, height: 24, borderRadius: 12, backgroundColor: SAGE, alignItems: "center", justifyContent: "center" },

  // Tab content wrapper
  tabContent: { padding: 16 },
  tabHeader: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 16 },
  tabHeaderIcon: { width: 28, height: 28, borderRadius: 8, backgroundColor: SAGE, alignItems: "center", justifyContent: "center", shadowColor: SAGE, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 6, elevation: 3 },
  tabTitle: { fontSize: 18, fontWeight: "800", color: SAGE, letterSpacing: -0.5 },

  // Count pill
  countPill: { backgroundColor: "rgba(79,95,82,0.1)", borderRadius: 999, paddingHorizontal: 8, paddingVertical: 2, borderWidth: 1, borderColor: "rgba(79,95,82,0.15)" },
  countPillText: { fontSize: 11, fontWeight: "700", color: SAGE },

  // Status badge
  statusBadge: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 9, paddingVertical: 4, borderRadius: 999, borderWidth: 1 },
  ...
  orderCard: {
    ...
    borderWidth: 1.5, borderColor: "rgba(242,237,228,0.9)",
    shadowColor: SAGE, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2,
  },
  orderCardAccent: { height: 3 },
  orderCardTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", padding: 14, paddingBottom: 10 },
  ...
  orderCardBottom: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", padding: 14, paddingTop: 10 },
  ...
  emptyIconBox: { width: 64, height: 64, borderRadius: 18, backgroundColor: "rgba(166,162,154,0.1)", borderWidth: 1.5, borderColor: "rgba(166,162,154,0.2)", borderStyle: "dashed", alignItems: "center", justifyContent: "center", marginBottom: 14 },
  ...
  // Profile
  profileCard: { backgroundColor: "#fff", borderRadius: 20, padding: 20, alignItems: "center", marginBottom: 12, borderWidth: 1.5, borderColor: "rgba(242,237,228,0.9)", shadowColor: SAGE, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
  profileAvatar: { width: 72, height: 72, borderRadius: 36, backgroundColor: SAGE, alignItems: "center", justifyContent: "center", marginBottom: 12, shadowColor: SAGE, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.25, shadowRadius: 10, elevation: 5 },
  ...
  verifCard: { backgroundColor: "#fff", borderRadius: 16, padding: 14, flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12, borderWidth: 1.5, borderColor: "rgba(242,237,228,0.9)" },
  ...
  stampsCard: { backgroundColor: "#fff", borderRadius: 16, padding: 16, marginBottom: 14, borderWidth: 1.5, borderColor: "rgba(242,237,228,0.9)" },
  ...
  stamp: { width: 34, height: 34, borderRadius: 17, backgroundColor: "rgba(166,162,154,0.12)", borderWidth: 1, borderColor: "rgba(166,162,154,0.2)", alignItems: "center", justifyContent: "center" },
  stampEarned: { backgroundColor: SAGE, borderColor: SAGE, shadowColor: SAGE, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4, elevation: 2 },
  stampsHint: { fontSize: 11, color: MUTED_GRAY },
  logoutBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 14, borderRadius: 16, borderWidth: 1.5, borderColor: "rgba(239,68,68,0.2)", backgroundColor: "rgba(239,68,68,0.05)" },
  logoutText: { fontSize: 15, fontWeight: "700", color: "#EF4444" },

  // Bottom tab bar
  tabBar: { flexDirection: "row", backgroundColor: "#fff", borderTopWidth: 1, borderTopColor: "rgba(242,237,228,0.9)", paddingBottom: 4, paddingTop: 6, shadowColor: "#000", shadowOffset: { width: 0, height: -2 }, shadowOpacity: 0.05, shadowRadius: 6, elevation: 8 },
  ...
  // Cart FAB
  cartFab: { position: "absolute", bottom: 80, right: 20, width: 56, height: 56, borderRadius: 28, overflow: "hidden", shadowColor: SAGE, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.32, shadowRadius: 12, elevation: 8 },
  cartFabGrad: { flex: 1, alignItems: "center", justifyContent: "center" },
  cartBadge: { position: "absolute", top: -2, right: -2, backgroundColor: "#EF4444", width: 20, height: 20, borderRadius: 10, alignItems: "center", justifyContent: "center", borderWidth: 1.5, borderColor: "#fff" },
  cartBadgeText: { color: "#fff", fontSize: 10, fontWeight: "800" },

  // Modal shared
  modalHandle: { width: 36, height: 4, borderRadius: 2, backgroundColor: "rgba(166,162,154,0.3)", alignSelf: "center", marginBottom: 8 },
  modalOverlay: { flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(20,28,22,0.5)" },
  modalHeader: { padding: 20, position: "relative", overflow: "hidden", borderTopLeftRadius: 24, borderTopRightRadius: 24 },
  modalHeaderBlob: { position: "absolute", width: 140, height: 140, borderRadius: 999, backgroundColor: "rgba(255,255,255,0.05)", top: -40, right: -30 },
  ...
  modalCloseBtn: { width: 32, height: 32, borderRadius: 10, backgroundColor: "rgba(255,255,255,0.12)", alignItems: "center", justifyContent: "center" },
  modalCancelBtn: { flex: 1, paddingVertical: 13, borderRadius: 14, borderWidth: 1.5, borderColor: "rgba(166,162,154,0.3)", alignItems: "center" },
  modalCancelText: { fontSize: 14, fontWeight: "600", color: MUTED_GRAY },
  modalAddBtn: { flex: 2, borderRadius: 14, overflow: "hidden", shadowColor: SAGE, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.25, shadowRadius: 8, elevation: 4 },
  modalAddBtnGrad: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 14 },
  modalAddBtnText: { color: "#fff", fontSize: 15, fontWeight: "700" },

  // Product modal
  productModal: { backgroundColor: "#fff", borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: "88%", paddingTop: 12 },

  // Options
  optionSection: { marginBottom: 18 },
  optionTitle: { fontSize: 12, fontWeight: "700", color: SAGE, letterSpacing: 0.6, textTransform: "uppercase", marginBottom: 10 },
  optionWrap: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  optionChip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 12, borderWidth: 1.5, borderColor: "rgba(166,162,154,0.25)", backgroundColor: "#FAFAFA", marginRight: 8 },
  optionChipActive: { backgroundColor: SAGE, borderColor: SAGE, shadowColor: SAGE, shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.2, shadowRadius: 6, elevation: 3 },
  ...
  qtyRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingVertical: 12, borderTopWidth: 1, borderTopColor: "rgba(242,237,228,0.8)" },
  qtyControls: { flexDirection: "row", alignItems: "center", gap: 12 },
  qtyBtn: { width: 36, height: 36, borderRadius: 12, backgroundColor: "rgba(79,95,82,0.08)", borderWidth: 1, borderColor: "rgba(79,95,82,0.12)", alignItems: "center", justifyContent: "center" },
  ...
  cartSheet: { backgroundColor: "#fff", borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingTop: 12, maxHeight: "92%" },
  cartHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: "rgba(242,237,228,0.8)" },
  cartHeaderIcon: { width: 28, height: 28, borderRadius: 8, backgroundColor: SAGE, alignItems: "center", justifyContent: "center" },
  cartHeaderTitle: { fontSize: 16, fontWeight: "800", color: SAGE },
  cartCloseBtn: { width: 32, height: 32, borderRadius: 10, backgroundColor: "rgba(166,162,154,0.1)", alignItems: "center", justifyContent: "center" },
  cartItem: { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: "rgba(242,237,228,0.6)" },
  ...
  cartRemoveBtn: { width: 28, height: 28, borderRadius: 8, backgroundColor: "rgba(239,68,68,0.08)", alignItems: "center", justifyContent: "center" },
  cartTotal: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 14 },
  ...
  // Schedule
  scheduleLabel: { fontSize: 10, fontWeight: "700", color: MUTED_GRAY, letterSpacing: 0.8, marginBottom: 8, textTransform: "uppercase" },
  schedulePill: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", backgroundColor: CREAM, borderRadius: 14, padding: 13, marginBottom: 10, borderWidth: 1, borderColor: "rgba(166,162,154,0.2)" },
  ...
  // Notes & place order
  notesInput: { backgroundColor: "#FAFAFA", borderRadius: 14, borderWidth: 1.5, borderColor: "rgba(166,162,154,0.25)", padding: 12, minHeight: 70, fontSize: 13, color: SAGE, textAlignVertical: "top", marginBottom: 16 },
  placeOrderBtn: { borderRadius: 16, overflow: "hidden", shadowColor: SAGE, shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.28, shadowRadius: 12, elevation: 6 },
  placeOrderGrad: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 16 },
